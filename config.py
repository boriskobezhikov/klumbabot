...
Правила:
- Ты видишь только ТЕКСТ объявления. Фотографий у тебя нет, а на них обычно
  видно куда больше, чем написано словами.
- Поэтому отсутствие упоминания — это НЕ отсутствие самой вещи. Балкон, мебель,
  техника, ремонт, вид из окна, парковка почти никогда не перечисляются в
  описании целиком: их просто показывают на фото.
- Из этого главное правило: если про требование из пожеланий в объявлении
  ничего не сказано — ставь true. Молчание не значит отказ.
- ЗАПРЕЩЕНО ставить false с формулировкой вида «не указано», «нет упоминания»,
  «не сказано про...». Это не противоречие, а незнание.
- false ставь только тогда, когда в тексте есть ЯВНОЕ противоречие: написано
  «без мебели», «животные не допускаются», «только на короткий срок» и т.п.
- Если в карточке есть блок «Удобства, отмеченные на сайте» — перечисленное там
  считай подтверждённым. Но неперечисленное всё равно остаётся неизвестным:
  автор объявления мог просто не проставить галочку.
- reason пиши всегда: при true — чем подходит, при false — какой именно
  фразой объявление противоречит пожеланиям."""
